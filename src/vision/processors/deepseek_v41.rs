...
//!
//! Port of the reference `load_image` from vLLM's
//! `models/deepseek_v4_1/common/mm_preprocess.py` (itself a port of the
...
//! [IMAGE_END]`; every span position carries `image_token_id` in `input_ids`
//! and the roles ride along in a per-image `types` tensor (the reference's
//! out-of-band `token_types`). IMAGE slots receive aligner rows in reading
//! order; the delimiters take the learned `image_start` / `image_newline` /
//! `image_end` vectors.
//!
//! One deviation from the reference token stream, mirroring vLLM: a leading
...
//! prepended when the block is spliced into the final prompt. `safe_resize`
//! reserves `COMPRESS_PAD_TO - 1` tokens of the budget for that pad.
...
impl GridPlan {
    /// Total number of LLM tokens the image span occupies (without the
    /// compressor-alignment pad).
...
fn safe_resize(
    ...
    let max_n_token = max_n_token - (COMPRESS_PAD_TO - 1);
...
fn image_token_types(n_llm_h: usize, n_llm_w: usize) -> Vec<i64> {
...
impl DeepseekV41Processor {
    ...
    /// geometry).
    fn plan_image_grid(&self, width: u32, height: u32) -> GridPlan {
    ...
    /// decoded (the frontend supplies images instead of a record dict).
    ...
    fn patchify(&self, image: &RgbImage, plan: &GridPlan) -> Array2<f32> {
        ...
        let patch_len = 3 * p * p;
        // TODO: parallelize over patch rows (rayon) if profiling shows the
        // sequential pass matters; ~9k patches/image worst case.
...
mod tests {
    ...
    /// Reference values computed with the vLLM port
    /// (`models/deepseek_v4_1/common/mm_preprocess.py`, patch=14,
    /// downsample=3, max_n_token=1024, min_pixels=295936, max_wh_ratio=None).
    #[test]
    fn plan_matches_reference() {
        // Small images are upscaled to min_pixels (a square of 544x544 =
        // 295936 pixels); ceil-to-patch makes the best size 546x546 and the
        // 13x13 token grid fits the budget.
        let p = plan(100, 100);
        assert_eq!((p.best_width, p.best_height), (546, 546));
        assert_eq!((p.n_vit_w, p.n_vit_h), (39, 39));
        assert_eq!((p.n_llm_w, p.n_llm_h), (13, 13));
        assert_eq!(p.num_image_tokens(), 184);

        // A large square image hits the token budget: solve_resize_ratio
        // picks the largest cell-aligned size whose grid fits in 1023 tokens.
        let p = plan(4000, 4000);
        assert_eq!((p.best_width, p.best_height), (1302, 1302));
        assert_eq!((p.n_vit_w, p.n_vit_h), (93, 93));
        assert_eq!((p.n_llm_w, p.n_llm_h), (31, 31));
        assert_eq!(p.num_image_tokens(), 994);

        // Non-square sizes stay aspect-preserving with gray padding.
        let p = plan(1024, 768);
        assert_eq!((p.best_width, p.best_height), (1036, 770));
        assert_eq!((p.n_llm_w, p.n_llm_h), (25, 19));
        assert_eq!(p.num_image_tokens(), 496);
    }

    #[test]
    fn extreme_aspects_collapse_to_single_strip() {
        // Extremely tall: solve_resize_ratio collapses to a single column.
        let p = plan(2, 1_000_000);
        assert_eq!(p.n_llm_w, 1);
        assert_eq!(p.num_image_tokens(), 1022);

        // Extremely wide: collapses to a single row.
        let p = plan(1_000_000, 2);
        assert_eq!(p.n_llm_h, 1);
        assert_eq!(p.num_image_tokens(), 1023);
    }
